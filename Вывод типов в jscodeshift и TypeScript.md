...
Начиная с версии 6.0 [jscodeshift](https://github.com/facebook/jscodeshift) поддерживает работу с TypeScript. В процессе написания codemode-ов (далее, преобразований), может потребоваться узнать тип переменной, которая не имеет явной аннотации. К сожалению, jscodeshift не предоставляет средств для вывода типов «из коробки».
...
К счастью, задача вывода типов уже решена внутри компилятора TypeScript. [API компилятора](https://github.com/microsoft/TypeScript/wiki/Using-the-Compiler-API) предоставляет средства для вывода типов, которые можно использовать для написания преобразования.

Однако, просто взять и воспользоваться компилятором TypeScript, переопределив парсер jscodeshift, нельзя. Дело в том, что jscodeshift ожидает от внешних парсеров дерево узлов в формате [ESTree](https://github.com/estree/estree). А дерево компилятора TS таковым не является.

Конечно, можно было бы воспользоваться API компилятора TS и без использования jscodeshift. Либо же воспользоваться одним из средств, которые существуют в комьюнити TS, например, [ts-morph](https://github.com/dsherret/ts-morph). Но для многих jscodeshift будет более привычным и выразительным решением. Поэтому далее будет рассмотрено, как обойти это ограничение.
...
Первый способ использует позиции узлов (номера строк и столбцов), чтобы найти отображение из TSTree в ESTree. Несмотря на то, что в общем случае позиции узлов могут не совпадать, почти всегда можно найти нужное отображение в каждом конкретном случае.
...
Сначала, нам нужно построить TSTree и получить `typeChecker` компилятора TypeScript:
...
Далее, построим отображение из ESTree в TSTree с использованием стартовой позиции. Для этого будем использовать двухуровневый `Map` (первый уровень – для строк, второй уровень – для столбцов, результат – узел TS-дерева):
...
Далее нам надо обойти все функции и методы классов, проверить возвращаемый тип и если он равен `null`, добавить аннотацию:
...
Пришлось скорректировать код для получения узла метода класса, т.к. по стартовой позиции узла метода ESTree в TSTree находится узел идентификатора метода.
...
Как было показано выше, хоть и отображение с использованием позиций узлов работает, оно не дает точного результата и требует ручной доводки. Более общим решением было бы написать явное отображение узлов TSTree в ESTree. Именно так работает парсер проекта [typescript-eslint](https://github.com/typescript-eslint/typescript-eslint).
...
Чтобы получить отображение узлов TSTree в ESTree и `typeChecker`, необходимо воспользоваться функцией `parseAndGenerateServices`. При этом мы должны передать в функцию путь к `.ts` файлу и путь к файлу конфигурации `tsconfig.json`. Так как прямого способа сделать этого нет, придется воспользоваться глобальной переменной (ох!):
```javascript
const parserState = {};
...
Каждый раз, когда мы хотим получить расширенный набор средств парсера typescript-eslint, мы вызываем функцию `parseWithServices`, в которую передаем все необходимые параметры (в остальных случаях мы по-прежнему используем функцию `j`):
```javascript
...
Надо отметить, что нам пришлось воспользоваться селектором `MethodDefinition` вместо `ClassMethod`, чтобы обойти методы классов (также немного изменился код доступа к возвращаемому значению метода). Это специфика парсера typescript-eslint. Код функции `getReturnType` идентичен тому, что использовался ранее.
...
Минусы:
 * Отображение узлов по позициям неточное и в некоторых случаях требует ручной доводки.
 ...
 * Необходимость использовать файлы для передачи конфигурации TypeScript и исходных текстов.

### Заключение

Оба подхода имеют свои плюсы и минусы.

Первый подход легко добавить в существующие проекты, т.к. он не требует переопределения парсера, но отображение узлов AST, скорее всего, потребует ручной доводки.
...
И создать программу TypeScript из строки:
